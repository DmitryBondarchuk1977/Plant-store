...
- Оформление: имя и фамилия подставляются из Telegram; телефон запрашивается через нативный
  шаринг контакта (`requestContact`) при нажатии «Оформить заявку»; есть поле для комментария.
- Повторный клиент (телефон уже сохранён) номер заново не вводит — поле заполнено, вместо кнопки
...
|-------|------|------------|
| POST | `/webhook` | апдейты Telegram: `/start` (inline-кнопка каталога), приём контакта как фолбэк |
...
- На `/start` номер **не** запрашивается — приходит только кнопка «Открыть каталог».
- Телефон запрашивается в Mini App в момент нажатия «Оформить заявку» через
  `Telegram.WebApp.requestContact()` (нативное окно Telegram). Полученный номер берётся из
  `event.responseUnsafe.contact.phone_number`.
- При отправке заявки номер сохраняется в `app_users.phone`.
- При следующих заказах `/me` возвращает сохранённый телефон — поле заполняется автоматически,
  кнопка шаринга прячется, остаётся ссылка «Изменить номер».
- Фолбэк: на клиентах без Bot API 8.0 `requestContact` недоступен — телефон вводится вручную.
